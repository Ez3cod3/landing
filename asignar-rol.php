			...
			<div class="wrapped">
				<div class="gray-bg" id="page-wrapper" style="min-height:182px;">
					<div class="row wrapper border-bottom white-bg page-heading">
						<div class="col-lg-10">
							<h2>Lista de roles por cada usuario</h2>
							<ol class="breadcrumb">
								<li><a href="home.php">Home</a></li>
								<li><a href="#">Lista de roles por cada usuario</a></li>
							</ol>
						</div>
							...
							<div class="col-lg-12">
								<div class="ibox float-e-margins">
									<div class="ibox-title">
										<h5> <i class="fa fa-briefcase"></i> Lista de usuarios asignados a un rol</h5>
									</div>
								</div>
								<div class="ibox-content">
										...
										<table class="table table-striped table-bordered table-hover dataTables-example dataTable" id="dataTables-example" role="grid" aria-describedby="dataTables-example_info">
											...
											<tbody>
												<?php 
													...
													$conexion = Conectar();
													$cont=0;
													$consulta="select * from user_rol as ur, rol as r, usuario as u where ur.CI_USER= u.CI_USER and ur.ID_ROL=r.ID_ROL";
													...
													while($dato=mysqli_fetch_array($query))
													{
														...
														<tr>
															<td >".$cont."</td>
															<td>".$dato["NOMB_ROL"]."</td>
															<td >".$dato["APE_PAT"]." ".$dato["APE_MAT"]." ".$dato["NOM_USER"]."</td>
															<td>".$dato["CI_USER"]."</td>
															<td>".$dato["COD_SIS"]."</td>
															<td>
																<div class='text-center'>
																	<a href='delete-asignar-rol.php?id=".$dato["ID_USER_ROL"]."'><span class='fa fa-times'></span></a></div></td>
														</tr>";
														}
													?>
											</tbody>
									...
									<div class="control-group">
										<div class="controls text-center"><a class="btn btn-success btn-w-m margin_bottom" href="#asignar" data-toggle="modal"><i class="fa fa-plus-square margin"></i>Nuevo</a></div>
									</div>
								</div>
							</div>
						</div>
					</div>
					<div class="modal inmodal" id="asignar" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
						<div class="modal-dialog" role="document">
							<div class="modal-content animated bounceInRight">
								<div class="modal-header">
									...
									<h3 class="modal-title">Asignar rol a usuarios</h3>
								</div>
								<div class="modal-body">
									<form class="form-signin" action="configure.php" method="post" enctype="multipart/form-data">
										...
										<div class="form-group">
											<label>Usuario</label>
											<div>
												<select class="chosen-select" name="user" tabindex="2">
													<option value="">Seleccionar</option>
													<?php 
														...
														while($dato=mysqli_fetch_array($query))
														{?>
														<option value="<?php echo $dato['CI_USER'];?>"><?php echo " {$dato['APE_PAT']} {$dato['APE_MAT']} {$dato['NOM_USER']}" ;?></option>
													<?php } ?>
												</select>
											</div>
										</div>
										<div class="modal-footer">
											<div class="text-center">
												<button class="btn btn-success btn-w-m" name="asignar-rol" type="input" id="asignar-rol"><i class="fa fa-check margin"> </i>Guardar Datos</button>
											</div>
										</div>
									</form>
								</div>
